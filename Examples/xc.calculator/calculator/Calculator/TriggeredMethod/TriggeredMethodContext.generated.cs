    ...
    public partial class TriggeredMethodContext : ICustomTriggeredMethodContext
    {
        ...
        public void Init(ComponentManager componentMgrInput)
        {
				...
				Environment.SetEnvironmentVariable("OWIN_SERVER", "Microsoft.Owin.Host.HttpListener.OwinServerFactory, XComponent.Core");
					var keyValueParameters = Enum.GetValues(typeof(KeyValueParametersEnum));
					foreach (var keyValueParameter in keyValueParameters)
					{
						var componentName = ComponentHelper.COMPONENT_NAME;
						var key = keyValueParameter.ToString();
						var value = GetParameterValue((KeyValueParametersEnum)keyValueParameter);
						FunctionsFactory.Instance.AddKeyValuePair(componentName, key, value);
					}
				try {
					Uri uri;
