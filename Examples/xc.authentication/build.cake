...
Task("Build")
  .Does(() =>
{
  foreach(var solution in GetFiles("./authenticationservice/**/*.sln"))
  {
    NuGetRestore(solution, new NuGetRestoreSettings { NoCache = true });
  }
  XcBuildBuild("./" + modelPath, buildConfiguration, "Dev", "VS2015", getXCBuildExtraParam());

  BuildNETSolution(@"authenticationservice\AuthenticationConsoleApp\AuthenticationConsoleApp.sln");
...
Task("GenerateRuntimeCmd")
  .Does(() => {
    ...
    var runClientAppCmd = "";
    runClientAppCmd += "cd authenticationservice/AuthenticationConsoleApp/AuthenticationConsoleApp/bin/Debug/\n";
