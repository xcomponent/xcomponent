...
var modelPath = Argument("modelPath", "benchsimplefork/BenchSimpleFork_Model.xcml");
...
Task("Build")
  .Does(() =>
{
  foreach(var solution in GetFiles("./benchsimplefork/**/*.sln"))
  {
    NuGetRestore(solution, new NuGetRestoreSettings { NoCache = true });
  }
  XcBuildBuild("./" + modelPath, buildConfiguration, "Dev", "VS2015", getXCBuildExtraParam());
  BuildNETSolution(@"benchsimplefork\BenchRunner\BenchRunner.sln");
...
Task("GenerateRuntimeCmd")
  .Does(() => {
    ...
    var runClientAppCmd = "";
    runClientAppCmd += "cd benchsimplefork/BenchRunner/BenchRunner/bin/Debug/\n";
    runClientAppCmd += "timeout /t 15\n";
    runClientAppCmd += "BenchRunner.exe\n";
