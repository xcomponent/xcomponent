...
var modelPath = Argument("modelPath", "slackproject/SlackGateway_Model.xcml");
...
Task("Build")
  .Does(() =>
{
  foreach(var solution in GetFiles("./slackproject/**/*.sln"))
  {
    NuGetRestore(solution, new NuGetRestoreSettings { NoCache = true });
  }
  XcBuildBuild("./" + modelPath, buildConfiguration, "Dev", "VS2015", getXCBuildExtraParam());
  BuildNETSolution(@"slackproject\SlackApp\SlackApp.sln");
...
Task("GenerateRuntimeCmd")
  .Does(() => {
    ...
    var runClientAppCmd = "";
    runClientAppCmd += "cd slackproject/SlackApp/SlackApp/bin/Debug/\n";
